...
### 5. Register your tool in tools.json

Open `tools.json` in the project root and add an entry to the `"tools"` array:
...
### 7. Sort and normalize tools.json

```bash
node sort-norm.js tools.json
```

This alphabetically sorts tools, normalizes tags (e.g., `js` becomes `javascript`, `html5` becomes `html`), and sorts tech stacks. Run this every time you edit `tools.json`.
...
> **Note:** You do **not** need to edit `index-template.txt`, `index.html`, or the ReadMe table. These are auto-generated by the build scripts. The maintainer runs `node sync-readme.js` after merging.
...
- [ ] Filename is kebab-case and matches the `id` in `tools.json`
- [ ] Screenshot saved as `tools/[id].png` (1280x720 recommended)
- [ ] Entry added to `tools.json` with all required fields
- [ ] Ran `node sort-norm.js tools.json`
- [ ] Ran `node build.js` and verified tool appears on landing page
